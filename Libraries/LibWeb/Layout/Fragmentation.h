...
public:
    ColumnFragmentationContext(GC::Ref<Box const> root_box, int column_count, CSSPixels column_width, CSSPixels column_gap, CSSPixels preferred_column_height, Optional<CSSPixels> max_column_height);
...
private:
    ...
    // m_preferred_column_height represents an ideal height which is calculated from the height the content would have
    // if flowed into a single, long column. Fragmentation breaks are assumed to sit at this ideal height.
    CSSPixels m_preferred_column_height;
    ...
    // preferred height for every column. In those cases, we either introduce a gap to shunt the monolithic content
    // across the break, into the next column, or we increase the actual column height to fit that content. If we
    // increased the actual height, we still want to fill all subsequent columns up to the preferred height, but we no
    // longer have enough content to do so. We call this a content deficit, and it is represented by this variable
    // being positive. If we introduce a gap, we now have too much content left. This amount is indicated by negative
    // values and we refer to it as a content surplus.
    // All fragmentation decisions are made to minimize both deficit and surplus.
